...
/**
 * 从API响应中提取预约信息
 * @param {string} responseData - API响应数据
 * @returns {Object} 包含预约时间和总数的对象
 */
  ...
  try {
    ...
    if (data.resultData?.content?.length) {
      bookingTimes = data.resultData.content.map(
        (item) => `预约日期:${item.reserveDate}\t预约时间:${item.reserveTime}`
      );
    }
 ...
 * @param {number} maxPages - 最大查询页数
 * @returns {Promise<Object>} 包含所有预约时间和总数的对象
 */
async function makeRequestWithPages(startPage = 1, maxPages = 5) {
  ...
  console.log(
    `开始查询预约信息，从第${startPage}页开始，最多查询${maxPages}页`
  );
  ...
  try {
    while (hasMoreData && currentPage < startPage + maxPages) {
      ...
      console.log(`正在查询第${currentPage}页...`);
      ...
      if (result.bookingTimes.length > 0) {
        ...
        console.log(
          `第${currentPage}页获取到${result.bookingTimes.length}条预约`
        );
        currentPage++;
        ...
        console.log(`第${currentPage}页没有预约记录，停止查询`);
        hasMoreData = false;
      }
    }

    // 发送通知
    const notifyContent = `✅ 查询成功: \n总预约次数: ${totalElements}\n最近十天预约时间列表:\n${allBookingTimes.join(
      "\n"
    )}`;
    ...
    if ($.isNode()) {
      if (typeof notify !== "undefined") {
        ...
        await $.msg($.name, `✅ 查询成功`, notifyContent);
      }
    }
    ...
    const errorMsg = `❌ 分页查询失败: ${error.message}`;
    if ($.isNode()) {
      if (typeof notify !== "undefined") {
        await notify.sendNotify($.name, errorMsg);
      } else {
        await $.msg($.name, `❌ 查询失败`, error.message);
      }
    }
    throw error;
  }
